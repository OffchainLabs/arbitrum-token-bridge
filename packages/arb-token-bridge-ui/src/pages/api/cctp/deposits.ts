...
export default async function handler(
  ...
  try {
    ...
    const messageSentsQuery = l1Subgraph.query({
      query: gql(`{
        messageSents(
          where: {            
    ...
    const messageReceivedQuery = l2Subgraph.query({
      query: gql(`{
        messageReceiveds(
          where: {          
    ...
    // MessageSents from L1 can be link to MessageSents on L2 with the tuple (sourceDomain, nonce)
