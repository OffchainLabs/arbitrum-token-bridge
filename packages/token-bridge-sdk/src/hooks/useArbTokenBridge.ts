...
import { getCachedL1TokenData, isClassicL2ToL1TransactionEvent } from '../util'
...
export const useArbTokenBridge = (
  ...
  async function depositToken({
    ...
    try {
      const { symbol, decimals } = await getCachedL1TokenData({
        account: walletAddress,
        erc20L1Address,
        l1Provider: l1.provider,
        l2Provider: l2.provider
  ...
  async function addToken(erc20L1orL2Address: string) {
    ...
    const bridgeTokensToAdd: ContractStorage<ERC20BridgeToken> = {}
    const { name, symbol, decimals } = await getCachedL1TokenData({
      account: walletAddress,
