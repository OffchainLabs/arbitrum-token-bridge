...
export const useArbTokenBridge = (
  ...
  const approveToken = async ({
    ...
    const tokenData =
      ...
      (await getL1TokenData(erc20L1Address))
  ...
  async function withdrawToken({
    ...
    const { symbol, decimals } = await (async () => {
      ...
      const { symbol, decimals } = searchTokenFromList(erc20L1Address) || (await getL1TokenData(erc20L1Address));
  ...
  async function triggerOutboxToken({
    ...
    const { symbol, decimals } =
      ...
      (await getL1TokenData(tokenAddress as string))
  ...
  const getTokenSymbol = async (_l1Address: string) => {
    ...
    try {
      const { symbol } =
        ...
        (await getL1TokenData(l1Address, false))
