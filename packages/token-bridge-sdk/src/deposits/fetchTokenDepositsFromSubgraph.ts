...
type QueryResult = {
  data: {
    gatewayDepositDatas: GatewayDepositData[]
  }
}

export type FetchTokenDepositsFromSubgraphResult = L2ToL1TransactionEvent & {
  l2TxHash: string
  amount: BigNumber
  tokenAddress: string
}
...
/**
 * Fetches initiated token deposits from subgraph in range of [fromBlock, toBlock].
 *
 ...
 */
export async function fetchTokenDepositsFromSubgraph({
  address,
  fromBlock,
  toBlock,
  l2Provider
  ...
  toBlock: number
  l2Provider: Provider
}): Promise<FetchTokenDepositsFromSubgraphResult[]> {
  ...
  const result: (FetchTokenDepositsFromSubgraphResult | null)[] =
    await Promise.all(
      queryResult.data.gatewayDepositDatas.map(async data => {
