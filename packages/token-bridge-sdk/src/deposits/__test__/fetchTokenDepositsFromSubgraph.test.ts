...
} from './fetchTokenDepositsTestHelpers'
import { fetchTokenDepositsFromSubgraph } from '../fetchTokenDepositsFromSubgraph'

describe('fetchTokenDepositsFromSubgraph', () => {
  it('fetches no token deposits from subgraph pre-nitro', async () => {
    const result = await fetchTokenDepositsFromSubgraph(
      getQueryCoveringClassicOnlyWithoutResults()
    )
  ...
  it('fetches some token deposits from subgraph pre-nitro', async () => {
    const result = await fetchTokenDepositsFromSubgraph(
      getQueryCoveringClassicOnlyWithResults()
    )
  ...
  it('fetches some token deposits from subgraph pre-nitro and post-nitro', async () => {
    const result = await fetchTokenDepositsFromSubgraph(
      getQueryCoveringClassicAndNitroWithResults()
    )
