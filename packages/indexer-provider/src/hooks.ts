...
export function useArbitrumIndexer(address: string): UseArbitrumIndexerResult {
  ...
  const { pending: pendingTransfers, completed: completedTransfers } = categorizeTransfers(
    allTransfers || []
  );
