...
const getLatestIndexedBlockNumber = async (subgraphName: string)=>{
    try {
      const res = await axios.post("https://api.thegraph.com/index-node/graphql", {"query":`{ indexingStatusForCurrentVersion(subgraphName: "${subgraphName}") { subgraph fatalError { message block { number } } nonFatalErrors {message } chains { network latestBlock { number }  } } }`})
      return res.data.data.indexingStatusForCurrentVersion.chains[0].latestBlock.number
    } catch(err){
